...
export default function SimulationPanel({ nearCanvasRef, farCanvasRef, simulated, busy, hasModes, totalPower, display, nearCrop, farCrop, onSimulate, onDownloadNear, onDownloadFar, onPredict, canPredict, modelSelected, predicting, modeCountMatches, modelNumModes, simModeCount }: { nearCanvasRef: RefObject<HTMLCanvasElement | null>; farCanvasRef: RefObject<HTMLCanvasElement | null>; simulated: boolean; busy: boolean; hasModes: boolean; totalPower: number; display: DisplaySettings; nearCrop: CropInfo | null; farCrop: CropInfo | null; onSimulate: () => void; onDownloadNear: () => void; onDownloadFar: () => void; onPredict: () => void; canPredict: boolean; modelSelected: boolean; predicting: boolean; modeCountMatches: boolean; modelNumModes: number | null; simModeCount: number }) {
  return <section className="panel simulation-panel">
    <div className="panel-heading compact"><div><p className="section-kicker">STEP 03</p><h2><StepBadge number={3} done={simulated}/>仿真单张光斑</h2></div><span className="resolution-chip">{display.outputSize} × {display.outputSize}</span></div>
    <div className="simulation-planes">
      <article className="plane-card"><div className="plane-title"><strong>近场</strong><span>|E|² · JET</span></div><div className={`canvas-frame ${simulated ? "has-result" : ""}`}><canvas ref={nearCanvasRef} width={display.outputSize} height={display.outputSize} aria-label="Jet 伪彩色近场光斑"/></div><small>{nearCrop && display.autoCrop ? `裁剪比例 ${nearCrop.cropRatio.toFixed(3)}` : "完整计算视场"}</small></article>
      <article className="plane-card"><div className="plane-title"><strong>远场</strong><span>FFT · JET</span></div><div className={`canvas-frame ${simulated ? "has-result" : ""}`}><canvas ref={farCanvasRef} width={display.outputSize} height={display.outputSize} aria-label="Jet 伪彩色远场光斑"/></div><small>{farCrop && display.autoCrop ? `裁剪比例 ${farCrop.cropRatio.toFixed(3)}` : "完整频域视场"}</small></article>
    </div>
    ...
    <div className="crop-summary"><span className={display.autoCrop ? "enabled" : ""}>{display.autoCrop ? (display.cropMode === "fixed" ? "使用自定义组合的统一固定裁剪" : `近远场均按 ${(display.energyFraction * 100).toFixed(1)}% 能量裁剪`) : "自动裁剪已关闭"}</span><strong>{nearCrop && display.autoCrop ? `近场视场 ${nearCrop.physicalWidth.toFixed(2)} μm` : "完整计算视场"}</strong></div>
    ...
    <button className={`predict-button full ${canPredict ? "ready" : "waiting"}`} onClick={onPredict} disabled={!modelSelected || busy || predicting}><span className="ai-icon">✦</span>{predicting ? "模型预测中…" : "模态预测（AI 模式分解）"}</button>
