...
const PYTHON_CANDIDATES = [
  ...
  join(PROJECT_DIR, ".venv", process.platform === "win32" ? "Scripts/python.exe" : "bin/python"),
  process.env.CONDA_PREFIX ? join(process.env.CONDA_PREFIX, process.platform === "win32" ? "python.exe" : "bin/python") : null,
  "python3",
  "python",
  "py",
].filter(Boolean);
...
function pythonHint() {
  return "未找到可用的 Python 环境（需要 torch / torchvision / timm / numpy / scipy / Pillow）。请在 启动后端.bat 中设置 LP_PREDICT_PYTHON（例如指向 conda 环境的 python.exe），或在 README 中查看配置说明。";
}
